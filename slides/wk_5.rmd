...
To do two-factor contrasts we will have to use the `emmeans` package. You might find this easier than the previous approaches where I had you specify your coding in a matrix. Whether you prefer this for one-way contrasts will be up to you.
---
...
Now we can specify our contrasts. This coding references slides 445-446 in Lab 5. I can run the contrasts so it prints the coding scheme.
...
If I was planning to put them into a markdown document, I'd probably want to write it all out with one command and output. I'm also going to add a 
```{r}
contrast(emm, method = list(
  "Hyp1" = (A1B1 - (A1B2 + A1B3)/2) -
  (A2B1 - (A2B2 + A2B3)/2),
